...
public class CellLayout extends ViewGroup {
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        if (child instanceof BubbleTextView) {
            ...
            bubbleChild.setTextVisibility(true);//wbin add
        }
    ...
    private boolean addViewToTempLocation(View v, Rect rectOccupiedByPotentialDrop,
        ...
        logD("[addViewToTempLocation] (View, Rect, int[] direction, itemconfig)");
        logD("[addViewToTempLocation] direction [" + direction[0]
                + ", " + direction[1] + "]");
        CellAndSpan c = currentState.map.get(v);
        logD("[addViewToTempLocation] view x=" + c.x + ", y="+ c.y
                + ", spanX=" + c.spanX + ", spanY=" + c.spanY);
        ...
        logD("[addViewToTempLocation] mTempLocation={"
                + mTempLocation[0] + ", " + mTempLocation[1] + "}");
        if (mTempLocation[0] >= 0 && mTempLocation[1] >= 0) {
            c.x = mTempLocation[0];
            c.y = mTempLocation[1];
            success = true;
        }
        logD("[addViewToTempLocation] success = " + success);
    ...
    private boolean pushViewsToTempLocation(ArrayList<View> views, Rect rectOccupiedByPotentialDrop,
        ...
        logD("[pushViewsToTempLocation] views.size() " + views.size()
        ...
        if (direction[0] < 0) {
            ...
            logD("[pushViewsToTempLocation] direction to LEFT" +
                    ", pushDistance = " + pushDistance);
        } else if (direction[0] > 0) {
            whichEdge = ViewCluster.RIGHT;
            pushDistance = rectOccupiedByPotentialDrop.right - clusterRect.left;
            logD("[pushViewsToTempLocation] direction to RIGHT" +
                    ", pushDistance = " + pushDistance);
        } else if (direction[1] < 0) {
            whichEdge = ViewCluster.TOP;
            pushDistance = clusterRect.bottom - rectOccupiedByPotentialDrop.top;
            logD("[pushViewsToTempLocation] direction to TOP" +
                    ", pushDistance = " + pushDistance);
        } else {
            whichEdge = ViewCluster.BOTTOM;
            pushDistance = rectOccupiedByPotentialDrop.bottom - clusterRect.top;
            logD("[pushViewsToTempLocation] direction to BOTTOM" +
                    ", pushDistance = " + pushDistance);
        }

        // Break early for invalid push distance.
        if (pushDistance <= 0) {
            logD("[pushViewsToTempLocation] invalid pushDistance, return.");
            return false;
        }

        // Mark the occupied state as false for the group of views we want to move.
        logD("[pushViewsToTempLocation] Mark the occupied state as" +
                " false for the group of views we want to move.");
        for (View v: views) {
            CellAndSpan c = currentState.map.get(v);
            markCellsForView(c.x, c.y, c.spanX, c.spanY, mTmpOccupied, false);
        }
            ...
            for (View v: currentState.sortedViews) {
                ...
                if (!cluster.views.contains(v) && v != dragView) {
                    if (cluster.isViewTouchingEdge(v, whichEdge)) {
                        ...
                        markCellsForView(c.x, c.y, c.spanX, c.spanY, mTmpOccupied, false);
                        logD("[pushViewsToTempLocation] cluster.addView" +
                                " and markCellsForView " + c.x + ", " + c.y
                                + ", " + c.spanX + ", " + c.spanY);
                    }
                }
            }
            logD("[pushViewsToTempLocation] pushDistance = " + pushDistance);
            pushDistance--;
        ...
        if (!fail && clusterRect.left >= 0 && clusterRect.right <= mCountX && clusterRect.top >= 0 &&
            ...
            logD("[pushViewsToTempLocation] foundSolution true ");
            foundSolution = true;
        } else {
        ...
        logD("[pushViewsToTempLocation] mark cluster views to true ");
    ...
    private boolean addViewsToTempLocation(ArrayList<View> views, Rect rectOccupiedByPotentialDrop,
        ...
        logD("[addViewsToTempLocation] views.size "
                + views.size() + " direction={" + direction[0] + ", " + direction[1]
                        + "}, dragView =" + dragView);
        boolean success = false;
        ...
        logD("[addViewsToTempLocation] boundingRect =["
        ...
        logD("[addViewsToTempLocation] mTempLocation =[" + mTempLocation[0]
                + ", " + mTempLocation[1] + "]");
        if (mTempLocation[0] >= 0 && mTempLocation[1] >= 0) {
            /* Begin: Deleted by bu.xinchun 2016-07-20, use apple reorder style,
             * for QMobile project requirement. */
            ...
            /* End: Deleted by bu.xinchun 2016-07-20, use apple reorder style,
             * for QMobile project requirement. */

            /* Begin: Added by bu.xinchun 2016-07-20, use apple reorder style,
             * for QMobile project requirement. */
            if (views.size() > 1) {
                int deltaX = mTempLocation[0] - boundingRect.left;
                int deltaY = mTempLocation[1] - boundingRect.top;
                ...
                int deltaX = mTempLocation[0] - boundingRect.left;
                int deltaY = mTempLocation[1] - boundingRect.top;
                View inter = views.get(0);
                CellAndSpan cTargetDrop = currentState.map.get(inter);
                empty[0] = cTargetDrop.x + deltaX;
                empty[1] = cTargetDrop.y + deltaY;
                logD("==[addViewsToTempLocation] interView = " + inter
                        + ", c.x = " + cTargetDrop.x + ", c.y = " + cTargetDrop.y
                        + ", deltaX = " + deltaX + ", deltaY = " + deltaY);

                target[0] = rectOccupiedByPotentialDrop.left;
                target[1] = rectOccupiedByPotentialDrop.top;

                logD("==[addViewsToTempLocation] empty[] = ["
                        + empty[0] + ", " + empty[1] + "], target[] = ["
                        + target[0] + ", " + target[1] + "]"
                        + ", mCountX=" + mCountX + ", mCountY=" + mCountY);

                boolean wrap;
                int startX;
                int endX;
                int startY;
                logD("rectOccupiedByPotentialDrop.left = " + rectOccupiedByPotentialDrop.left
                        + ", rectOccupiedByPotentialDrop.top = " + rectOccupiedByPotentialDrop.top);
                if (readingOrderGreaterThan(target, empty)) {
                    logD("Target cell order is greater than empty cell.");
                    wrap = empty[0] >= mCountX - 1;
                    startY = wrap ? empty[1] + 1 : empty[1];
                    for (int y = startY; y <= target[1]; y++) {
                        startX = y == empty[1] ? empty[0] + 1 : 0;
                        endX = y < target[1] ? mCountX - 1 : target[0];
                        for (int x = startX; x <= endX; x++) {
                            View v = getChildAt(x,y);
                            logD("x=" + x + ", y=" + y + ", view=" + v);

                            CellAndSpan c = currentState.map.get(v);
                            if (c == null || c.spanX > 1 || c.spanY > 1) { // skip widget item.
                                continue;
                            }
                            ...
                            logD("before swap index with empty, =8888=,"
                                    + ", cell=[" + c.x + ", " + c.y
                                    + ", " + (c.x + c.spanX)
                                    + ", " + (c.y + c.spanY) + "]"
                    ...
                    logD("target cell order is less than empty cell.");
                    wrap = empty[0] == 0;
                    startY = wrap ? empty[1] - 1 : empty[1];
                    for (int y = startY; y >= target[1]; y--) {
                        startX = y == empty[1] ? empty[0] - 1 : mCountX - 1;
                        ...
                        for (int x = startX; x >= endX; x--) {
                            ...
                            logD("before swap index with empty, =9999=,"
                                    + ", cell=[" + c.x + ", " + c.y
                                    + ", " + (c.x + c.spanX)
                                    + ", " + (c.y + c.spanY) + "]"
                                    + ", empty[]=[" + empty[0] + ", " + empty[1] + "]");
            ...
            /* End: Added by bu.xinchun 2016-07-20, use apple reorder style,
             * for QMobile project requirement. */
            success = true;
        }
        ...
        logD("[addViewsToTempLocation] success = " + success);
        return success;
    }
    ...
    static final boolean IS_ENABLE_LOGD = false;
    private void logD(String logTxt) {
        if (!IS_ENABLE_LOGD) return;
        Log.d("bxc", logTxt);
    }
    // End: Added by bu.xinchun
    ...
    private boolean attemptPushInDirection(ArrayList<View> intersectingViews, Rect occupied,
        ...
        logD("[attemptPushInDirection] intersectingViews, occupied, direction, ignoreView, solution");
        logD("[attemptPushInDirection] direction[0]=" + direction[0]
                + ", direction[1]=" + direction[1]);
        if ((Math.abs(direction[0]) + Math.abs(direction[1])) > 1) {
            /* Begin: Deleted by bu.xinchun 2016-07-20, for Apple reorder style,
             * remove complex push.*/
            /*
            // If the direction vector has two non-zero components, we try pushing
            // separately in each of the components.
            logD("[attemptPushInDirection] abs direc > 1" +
                    ", try pushing separately in each of the components.");
            int temp = direction[1];
            direction[1] = 0;

            logD("[attemptPushInDirection] try to push in X-axis.");
            if (pushViewsToTempLocation(intersectingViews, occupied, direction,
                    ignoreView, solution)) {
                logD("[attemptPushInDirection] push finished. 2188");
                return true;
            }
            direction[1] = temp;
            temp = direction[0];
            ...
            logD("[attemptPushInDirection] try to push in Y-axis.");
            if (pushViewsToTempLocation(intersectingViews, occupied, direction,
                    ignoreView, solution)) {
                logD("[attemptPushInDirection] push finished. 2198");
                return true;
            }
            ...
            temp = direction[1];
            direction[1] = 0;
            logD("[attemptPushInDirection] try to push opposite in X-axis.");
            if (pushViewsToTempLocation(intersectingViews, occupied, direction,
                    ignoreView, solution)) {
                logD("[attemptPushInDirection] push finished. 2212");
                return true;
            }
            ...
            temp = direction[0];
            direction[0] = 0;
            logD("[attemptPushInDirection] try to push opposite in Y-axis.");
            if (pushViewsToTempLocation(intersectingViews, occupied, direction,
                    ignoreView, solution)) {
                logD("[attemptPushInDirection] push finished. 2222");
                return true;
            }
            // revert the direction
            direction[0] = temp;
         ...
         * remove complex push.*/
            ...
            logD("[attemptPushInDirection]  single non-zero direc" +
                    ", try pushing separately in each of the components.");
            logD("[attemptPushInDirection] push first in the direction of the vector.");
            if (pushViewsToTempLocation(intersectingViews, occupied, direction,
                    ignoreView, solution)) {
                logD("[attemptPushInDirection] push finished. 2238");
                return true;
            }
            // Then we try the opposite direction
            direction[0] *= -1;
            direction[1] *= -1;
            logD("[attemptPushInDirection] try the opposite direction.");
            if (pushViewsToTempLocation(intersectingViews, occupied, direction,
                    ignoreView, solution)) {
                logD("[attemptPushInDirection] push finished. 2247");
                return true;
            }
            ...
            direction[0] = temp;
            logD("[attemptPushInDirection] swap direction x, y. then push.");
            if (pushViewsToTempLocation(intersectingViews, occupied, direction,
                    ignoreView, solution)) {
                logD("[attemptPushInDirection] push finished. 2264");
                return true;
            }
            ...
            if (pushViewsToTempLocation(intersectingViews, occupied, direction,
                    ignoreView, solution)) {
                logD("[attemptPushInDirection] push finished. 2274");
                return true;
            }
            // Switch the direction back
            direction[0] *= -1;
        ...
        logD("[attemptPushInDirection] push FAIL 2286.");
        return false;
    }

    private boolean rearrangementExists(int cellX, int cellY, int spanX, int spanY, int[] direction,
            View ignoreView, ItemConfiguration solution) {
        logD("[rearrangementExists] cellX = " + cellX + ", cellY = " + cellY
        ...
        int [] dragSrc = null; // Added by bu.xinchun
        ...
        logD("[rearrangementExists] mIntersectingViews.size() = "
                + mIntersectingViews.size());
        ...
        if (attemptPushInDirection(mIntersectingViews, mOccupiedRect, direction, ignoreView,
                solution)) {
            logD("[rearrangementExists] attemptPushInDirection() return true====");
            return true;
        }

        // Next we try moving the views as a block, but without requiring the push mechanic.
        logD("[rearrangementExists] try moving the views as a block. ");
        if (addViewsToTempLocation(mIntersectingViews, mOccupiedRect, direction, ignoreView,
                solution)) {
            logD("[rearrangementExists] addViewsToTempLocation(" +
                    "ArrayList<View> views, Rect rectOccupiedByPotentialDrop" +
                    ", int[] direction, View dragView" +
                    ", ItemConfiguration currentState) return true====");
            return true;
        } else {
            logD("[rearrangementExists] addViewsToTempLocation(" +
                    "ArrayList<View> views, Rect rectOccupiedByPotentialDrop" +
                    ", int[] direction, View dragView" +
                    ", ItemConfiguration currentState) return false");
        }

        // Ok, they couldn't move as a block, let's move them individually
        logD("[rearrangementExists] they couldn't move as a block, let's move them individually. ");
        for (View v : mIntersectingViews) {
            if (!addViewToTempLocation(v, mOccupiedRect, direction, solution)) {
                logD("[rearrangementExists] addViewToTempLocation(View v" +
                        ", Rect rectOccupiedByPotentialDrop, int[] direction" +
                        ", ItemConfiguration currentState). return false");
                return false;
            }
        }
        logD("[rearrangementExists] move them individually finished !");
        return true;
    }
    ...
    ItemConfiguration findReorderSolution(int pixelX, int pixelY, int minSpanX, int minSpanY,
        ...
        logD("[findReorderSolution] pixelX = " + pixelX + ", pixelY = "
                + pixelY + ", minSpanX = " + minSpanX + ", minSpanY = " + minSpanY
            + ", spanX = " + spanX + ", spanY = " + spanY
            + ", direction ={" + direction[0] + ", " + direction[1]
            + ", dragView = " + dragView + ", decX = " + decX);
        result = findNearestArea(pixelX, pixelY, spanX, spanY, result);
        logD("[findReorderSolution] findNearestArea() "
                + " result = {" + result[0] + ", " + result[1] + "}");

        boolean success = false;
        // First we try the exact nearest position of the item being dragged,
        // we will then want to try to move this around to other neighbouring positions
        logD("[findReorderSolution] rearrangementExists() invoke begin.");
        logD("print current layout :");
        printItemConfiguration(solution);
        success = rearrangementExists(result[0], result[1], spanX, spanY, direction, dragView,
                solution);
        logD("rearrangement " + success + ", print new layout :");
        printItemConfiguration(solution);
        logD("[findReorderSolution] rearrangementExists() return " + success);
    ...
    */
    ...
    int[] performReorder(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
        ...
        if (swapSolution.isSolution && swapSolution.area() >= noShuffleSolution.area()) {
            ...
            logD("[performReorder] use swap solution.");
        } else if (noShuffleSolution.isSolution) {
            finalSolution = noShuffleSolution;
            logD("[performReorder] use noShuffleSolution.");
        }
    ...
    // Begin: Added for debug by bu.xinchun 2016-07.
    private void printItemConfiguration(ItemConfiguration itemConfig) {
        if (itemConfig == null) {
            logD("[printItemConfiguration] itemConfig is NULL.");
            return;
        }
        StringBuffer sb = new StringBuffer();
        for (int i = 0; i < itemConfig.sortedViews.size(); i++) {
            View v = itemConfig.sortedViews.get(i);
            CellAndSpan cell = itemConfig.map.get(v);
            if (cell == null) {
                sb.append("[N, N, N, N] null\t");
            } else {
        ...
        logD(sb.toString());
        logD("[printItemConfiguration] itemConfig print end -----");
    }
    // End: Added for debug by bu.xinchun 2016-07.
