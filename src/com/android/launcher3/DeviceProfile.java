...
public class DeviceProfile {
    ...
    DeviceProfile(Context context,
        ...
        desiredWorkspaceLeftRightMarginPx =  4*edgeMarginPx;//wbin modify
		...
		if(LauncherAppState.isDisableAllApps()){
		    numHotseatIcons = numHotseatIcons - 1;
		}
    ...
    private void updateIconSize(float scale, int drawablePadding, Resources resources,
        ...
        // Hotseat
        hotseatBarHeightPx =(int)((iconSizePx+4*edgeMarginPx)*1.1);//iconSizePx + 4 * edgeMarginPx;// (int)((iconSizePx+4*edgeMarginPx)*1.1);//
        hotseatCellWidthPx = iconSizePx; //iconSizePx;
        hotseatCellHeightPx =hotseatBarHeightPx;// hotseatBarHeightPx;//iconSizePx;

        // Folder
        folderCellWidthPx = 260;//cellWidthPx + 3 * edgeMarginPx;
        folderCellHeightPx = 260;//cellHeightPx + edgeMarginPx;
        ...
        if (orientation == CellLayout.LANDSCAPE &&
            ...
            if (isTablet()) {
                ...
                bounds.set(desiredWorkspaceLeftRightMarginPx - defaultWidgetPadding.left,
                       ...
                       50, searchBarSpaceHeightPx);//wbin modify
            }
        }
        return bounds;
    }
    ...
    public void layout(Launcher launcher) {
        ...
        if (hasVerticalBarLayout) {
            ...
            lp.gravity = Gravity.BOTTOM;
            lp.width = LayoutParams.MATCH_PARENT;
            lp.height = hotseatBarHeightPx+10;
            hotseat.findViewById(R.id.layout).setPadding(27 * edgeMarginPx, 0,
                    27* edgeMarginPx, 0);//wbin modify
        }
        ...
        if (overviewMode != null) {
            ...
            if (LauncherExtPlugin.getInstance().getWorkspaceExt(
                    launcher.getApplicationContext()).supportEditAndHideApps()) {
                final ViewGroup overviewModeChild = (ViewGroup) overviewMode.getChildAt(0);
                lp.width = Math.min(availableWidthPx,
                        calculateOverviewModeWidth(getVisibleChildCount(overviewModeChild)));
            } else {
                lp.width = Math.min(availableWidthPx,
                        calculateOverviewModeWidth(getVisibleChildCount(overviewMode)));
            }

            lp.height = r.height()+20;
