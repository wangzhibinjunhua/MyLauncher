...
class AllAppsList {
     ...
     */
    public void add(AppInfo info,boolean child) {
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "Add application in app list: app = " + info.componentName
                    + ", title = " + info.title);
        }

        String packagename_inp = info.getPackageName();//wbin modify
        if(null != packagename_inp && !"".equals(packagename_inp)){
            if(packagename_inp.equals("com.android.bguistore") || packagename_inp.equals("com.android.bglauncher")){
				return;
            	}
        }
     ...
     */
    public void addPackage(Context context, String packageName, UserHandleCompat user,boolean child) {
        ...
        if (matches.size() > 0) {
            for (LauncherActivityInfoCompat info : matches) {
                add(new AppInfo(context, info, user, mIconCache, null),child);
            }
        }
    }
    ...
    public void removePackage(String packageName, UserHandleCompat user) {
        ...
        if (LauncherLog.DEBUG) {
            Log.e("wbin", "removePackage: packageName = " + packageName + ", data size = " + data.size());
        }
     ...
     */
    public  boolean displayOnStudent(String packageName,boolean child)
    	{
    		String[] oripackageName = {"com.application.actionparentspage",
									"com.trueease.sparklehome","net.xuele.wisdom.xuelewisdom",
									"com.application.protecteyesight","com.application.weburl",
									"com.application.huayingappstore","net.xuele.xuelets"};
			// "com.android.browser" "com.clovsoft.huayinghealth.teacher" "com.application.classschedule","com.android.settings","com.clovsoft.huayinghealth.student"
    ...
    public void updatePackage(Context context, String packageName, UserHandleCompat user,boolean child) {
        final LauncherAppsCompat launcherApps = LauncherAppsCompat.getInstance(context);
        final List<LauncherActivityInfoCompat> matches = launcherApps.getActivityList(packageName,
                user);
        if (LauncherLog.DEBUG) {
	...
	if (SystemProperties.get("persist.sys.usermode").equals("1"))
		{
	        if(!displayOnStudent(packageName,child))
	        	{
	        		return ;
	        	}
		}
        if (matches.size() > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (applicationInfo == null) {
                    add(new AppInfo(context, info, user, mIconCache, null),child);//wbin modify
                } else {
    ...
    static boolean loadTopPackage(final Context context) {
        ...
        try {
            ...
            while (((type = parser.next()) != XmlPullParser.END_TAG || parser.getDepth() > depth)
				...
				//wbin add
			if (SystemProperties.get("persist.sys.usermode").equals("1"))//student
				{
				if(a.getString(R.styleable.TopPackage_topPackageName).equals("com.application.actionstudentpage")|| 
					a.getString(R.styleable.TopPackage_topPackageName).equals("com.application.parentsmanage"))
					{
						a.recycle();
						loop ++;
					}
				else //parent
					{
			                sTopPackages.add(new TopPackage(a.getString(R.styleable.TopPackage_topPackageName),
			                        a.getString(R.styleable.TopPackage_topClassName), a.getInt(
			                                R.styleable.TopPackage_topOrder, 0)-loop));
				...
				{
					if(a.getString(R.styleable.TopPackage_topPackageName).equals("com.application.actionparentspage"))
						{
							a.recycle();
							loop ++;
						}
