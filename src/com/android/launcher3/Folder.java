...
public class Folder extends LinearLayout implements DragSource, View.OnClickListener,
	...
	private void setFolderBackground() {
		...
		Bitmap wallpaperBitmap = drawable2Bitmap(drawable);	
		int screenWidth = windowManager.getDefaultDisplay().getWidth();
		int screenHeight = windowManager.getDefaultDisplay().getHeight();
		int wallpaperWidth = wallpaperBitmap.getWidth();
		int wallpaperHeight = wallpaperBitmap.getHeight();	
		float scaleWidth = (float)wallpaperWidth/(float)screenWidth;
		float scaleHeight = (float)wallpaperHeight/(float)screenHeight;
		
        int width = 320;//getPaddingLeft() + getPaddingRight() + mContent.getDesiredWidth();
        int height = 320;//getContentAreaHeight();		
		
		int left = (screenWidth-width) / 2;
		int top = (screenHeight-height) / 2;		
		Bitmap bitmap = Bitmap.createBitmap(wallpaperBitmap, Math.round(left*scaleWidth), Math.round(top*scaleHeight), Math.round(width*scaleWidth), Math.round(height*scaleHeight));		
		...
		mScrollView.setBackground(bitmap2Drawable(getRoundImage(blurBitmap(bitmap, getContext()),34.0f)));	
   ...
   public Drawable bitmap2Drawable(Bitmap bitmap) {
    ...
    private void centerAboutIcon() {
		...
		left = (720-width) / 2;
		top = (1280-height - getPaddingTop() - mFolderNameHeight) / 2;
